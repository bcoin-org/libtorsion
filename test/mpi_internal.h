...
test_mpz_bits(mp_rng_f *rng, void *arg) {
  ...
  for (i = 0; i < 100; i++) {
    ...
    for (j = 0; j < bits; j++) {
      ASSERT(mpz_get_bit(x, j) == (y->limbs[0] & 1));
...
test_mpz_primes(mp_rng_f *rng, void *arg) {
...

  {
    ...
    for (i = 0; i < ARRAY_SIZE(composite_vectors); i++) {
      ...
      /* ASSERT(!mpz_is_prime_mr(p, 4, 1, rng, arg)); */
      /* ASSERT(!mpz_is_prime_mr(p, 4, 0, rng, arg)); */
